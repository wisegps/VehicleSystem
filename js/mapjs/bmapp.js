...
function bmap(div_map, center_point, zoom, opt) {
...
//  this.map.addControl(new BMap.ScaleControl());
    	...
    	if(this.opt.traffic){
    	    	var trafficOn = false;
 		var trafficLayer;
		var trafficCtrl = new ZoomControl(BMAP_ANCHOR_TOP_RIGHT, new BMap.Size(10, 40), '实时路况', './images/traffic_off.png', function(e){
			trafficOn = !trafficOn;
	...
	if(this.opt.trace){
		var traceOn = false;
		var traceCtrl = new ZoomControl(BMAP_ANCHOR_TOP_RIGHT, new BMap.Size(10, 85), '跟踪车辆', './images/locate_off.png', function(e){
//			traceOn = !traceOn;
			var target = e.target.localName == 'img' ? e.target: e.target.querySelector('img');
			target.stat = traceOn;
			target.on = function(){
	...
	// 添加我的位置
	if(this.opt.myPos){
		var myPosCtrl = new ZoomControl(BMAP_ANCHOR_TOP_RIGHT, new BMap.Size(10, 130), '我的位置', './images/default.png', function(e){
			var target = e.target.localName == 'img' ? e.target: e.target.querySelector('img');
	...
	// 添加刷新按钮
	if(this.opt.refresh){
		var refreshCtrl = new ZoomControl(BMAP_ANCHOR_TOP_RIGHT, new BMap.Size(10, 175), '车辆位置', './images/refresh.png', function(e){
			var target = e.target.localName == 'img' ? e.target: e.target.querySelector('img');
...
bmap.prototype.addVehicles = function (vehicles, is_infowin) {
    var v = null;
    var latLng = null;
    var icon = "";
    var title = "";
    for (var i = 0; i < vehicles.length; i++) {
        ...
        if (v != null) {
            ...
            icon = getIcon(vehicles[i], MAP_TYPE_BAIDU, false);
//          title = vehicles[i].obj_name + "（" + getStatusDesc(vehicles[i], 2) + "）";
            v.marker_ = new BMap.Marker(latLng, {icon: icon, offset: new BMap.Size(0, -15)});
            v.marker_.setLabel(new BMap.Label(vehicles[i].obj_name, {offset: new BMap.Size(30, 0)}));
...
//      var latLng = new BMap.Point(v.b_lon, v.b_lat);
...
bmap.prototype.updateVehicle = function (vehicle, if_track, if_show_line, if_open_win, color, width, if_playback) {
    ...
    if (v != null) {
        var oldlatLng;
        oldlatLng = new BMap.Point(v.b_lon, v.b_lat);
...
//      v.direct = vehicle.active_gps_data.direct;
        ...
        v.marker_.getLabel().setContent(vehicle.obj_name);
...
bmap.prototype.findVehicle = function (obj_id, if_track, if_open_win) {
    ...
    if (v != null) {
        var latLng;
        latLng = new BMap.Point(v.b_lon, v.b_lat);
